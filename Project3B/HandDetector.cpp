...
void HandDetector::DetectContours(cv::Mat& frame, cv::Mat& result_hsv, std::vector<std::vector<cv::Point>>& conts)
{
	cv::cvtColor(frame, frame_hsv, cv::COLOR_BGR2HSV);
	cv::inRange(frame_hsv, hsv_lower, hsv_upper, result_hsv);
	cv::medianBlur(result_hsv, result_hsv, 5);

	cv::findContours(result_hsv, conts, cv::RETR_LIST, cv::CHAIN_APPROX_NONE);
